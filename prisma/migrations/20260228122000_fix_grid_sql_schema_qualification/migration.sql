-- Normalize configured grid SQL to schema-qualified table names for non-public modules.
DO $$
DECLARE
  v_spec RECORD;
BEGIN
  ...
  LOOP
    ...
    WHERE gd.grid_is_deleted = false
      ...
      AND gd.grid_sql ~* format('(^|\\W)%s(\\W|$)', v_spec.table_name)
