...
describe('ItemsGroupMasterService', () => {
    ...
    beforeEach(() => {
        ...
        configuredGridSqlService = {
            loadCandidates: jest.fn().mockImplementation(async (options) => {
                const where = {
                    gridId: BigInt(1),
                    gridIsDeleted: false,
                    gridStatus: true,
            ...
            runPagedQuery: jest
                ...
                const rowsSql = `SELECT * FROM (${options.baseSql}) AS ${options.alias}_rows LIMIT $${params.length + 1} OFFSET $${params.length + 2}`;
                ...
                if (typeof totalRaw === 'bigint') {
                    return { items: rows, total: Number(totalRaw) };
                }
                if (typeof totalRaw === 'number') {
                    return { items: rows, total: Number.isFinite(totalRaw) ? totalRaw : 0 };
                }
                if (typeof totalRaw === 'string') {
                    const parsed = Number(totalRaw);
                    return { items: rows, total: Number.isFinite(parsed) ? parsed : 0 };
                }
        ...
        service = new items_group_master_service_1.ItemsGroupMasterService(prisma, auditLogService, requestContextService);
    });
    ...
    it('fails create when audit logging fails (fail-closed)', async () => {
        ...
        prisma.itemGroupMaster.update.mockResolvedValueOnce(createdWithPath);
        prisma.itemGroupMaster.findFirst.mockResolvedValueOnce(createdWithPath);
        auditLogService.logEntityChange.mockRejectedValueOnce(new Error('audit failed'));
    ...
    it('updates an item group when itg_id is provided', async () => {
        ...
        expect(updateArgs.data.itgModifiedBy).toBe(module_shared_utils_1.DEFAULT_ACTOR);
    ...
    it('rejects duplicate name with 409', async () => {
        ...
        await expect(service.save(input)).rejects.toBeInstanceOf(common_1.ConflictException);
    });
    it('rejects self-parent updates', async () => {
        ...
        await expect(service.save(input)).rejects.toBeInstanceOf(common_1.BadRequestException);
    });
    ...
    it('removes subtree ids from old ancestors when reparented to root', async () => {
        ...
        prisma.itemGroupMaster.findMany
            ...
            .mockResolvedValueOnce([refreshed])
            .mockResolvedValueOnce([oldParent]);
        prisma.itemGroupMaster.update.mockResolvedValueOnce(refreshed).mockResolvedValueOnce(makeRecord({
            itgId: PARENT_GROUP_ID,
            itgParentId: null,
            itgPathIdsCache: [PARENT_GROUP_ID],
        }));
    ...
    it('returns 404 in getById when row is missing or soft deleted', async () => {
        ...
        await expect(service.getById(ITEM_GROUP_ID)).rejects.toBeInstanceOf(common_1.NotFoundException);
    ...
    it('soft delete removes subtree ids from ancestor caches', async () => {
        ...
        prisma.itemGroupMaster.update.mockResolvedValueOnce(makeRecord({
            itgId: PARENT_GROUP_ID,
            itgParentId: null,
            itgPathIdsCache: [PARENT_GROUP_ID],
        }));
        ...
        expect(updateManyArgs.data.itgModifiedBy).toBe(module_shared_utils_1.DEFAULT_ACTOR);
        expect(prisma.itemGroupMaster.update).toHaveBeenCalledTimes(1);
