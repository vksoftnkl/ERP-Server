...
let ItemPriceDetailsService = class ItemPriceDetailsService {
    ...
    async getByItemId(itemId) {
        ...
        return {
            ...
            item_unit_conversions: unitConversionRecords.map((record) => this.toItemUnitConversionPayload(record)),
            item_tax: taxRecord ? this.toItemTaxPayload(taxRecord) : null,
        };
    }
    toItemUnitConversionPayload(record) {
        return {
            iuc_id: record.iucId,
    ...
    throwItemNotFound(itemId) {
        throw new common_1.NotFoundException(this.buildErrorResponse('Item not found', [
            {
                field: 'item_id',
