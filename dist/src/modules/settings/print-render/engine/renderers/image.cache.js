...
let ImageCache = ImageCache_1 = class ImageCache {
    ...
    constructor() {
        ...
        this.allowRemote = ['1', 'true', 'yes', 'on'].includes((process.env.PRINT_ALLOW_REMOTE_IMAGES ?? '').toLowerCase());
    ...
    async load(source) {
        ...
        if (/^https?:\/\//i.test(source)) {
            if (!this.allowRemote) {
                this.warn(`Remote image '${truncateForLog(source)}' refused. Set PRINT_ALLOW_REMOTE_IMAGES=true ` +
