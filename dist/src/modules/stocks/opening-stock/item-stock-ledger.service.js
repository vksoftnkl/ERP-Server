...
exports.ItemStockLedgerService = void 0;
...
const batch_number_util_1 = require("../../../common/utils/batch-number.util");
...
const ITEM_BATCH_STATUS_VALUES = ['ACTIVE', 'CLOSED', 'BLOCKED'];
const ITEM_BATCH_STATUS_SET = new Set(ITEM_BATCH_STATUS_VALUES);
const ITEM_STOCK_BUCKET_VALUES = Object.values(item_stock_types_1.ItemStockBucket);
const ITEM_STOCK_BUCKET_SET = new Set(ITEM_STOCK_BUCKET_VALUES);
const ITEM_BATCH_STOCK_STATE_SELECT = client_1.Prisma.validator()({
    ibsId: true,
    ibsAccYear: true,
    ibsCompanyId: true,
    ibsBranchId: true,
...
const ITEM_STOCK_BALANCE_STATE_SELECT = client_1.Prisma.validator()({
    isbId: true,
    isbAccYear: true,
    isbCompanyId: true,
    isbBranchId: true,
...
let ItemStockLedgerService = class ItemStockLedgerService {
    prisma;
    requestContextService;
    auditLogService;
    constructor(prisma, requestContextService, auditLogService) {
        this.prisma = prisma;
        this.requestContextService = requestContextService;
        this.auditLogService = auditLogService;
    }
    async syncFromOpeningStockDocument(tx, document, previousDocument) {
        if (document.details.length === 0) {
            throw new common_1.BadRequestException('Opening stock response must contain at least one detail row');
        }
        const actorUserId = document.header.osh_updated_by ??
        ...
        try {
            ...
            const resolvedPreviousDocument = previousDocument
                ...
                : undefined;
            const resolvedDocument = await this.resolveBatchIdsForDocument(tx, document, actorUserId, now, true);
            ...
            const itemStockLedger = await Promise.all(resolvedDocument.details.map((detail, index) => tx.itemStockLedger.create({
                data: this.buildLedgerCreateInput(resolvedDocument.header, detail, index, actorUserId, now),
            })));
            ...
            const { itemStockBalance, itemBatchStock } = await this.applyStockMovements(tx, [...reverseMovements, ...currentMovements], actorUserId, now);
            return {
                itemStockLedger,
                itemStockBalance,
                itemBatchStock,
    ...
    buildLedgerCreateInput(header, detail, index, actorUserId, now) {
        const voucherDate = this.parseRequiredDate(header.osh_voucher_date, 'header.osh_voucher_date');
        const createdOn = this.parseOptionalDate(detail.osl_created_on, `details[${index}].osl_created_on`) ?? now;
        ...
        return {
            ...
            stlTxnType: item_stock_types_1.StockTxnType.OPENING,
            stlStockEffect: 1,
            ...
            stlExpiryDate: this.parseOptionalDate(detail.osl_expiry_date, `details[${index}].osl_expiry_date`),
    ...
    buildOpeningStockMovements(document, direction) {
        const txnDate = this.parseRequiredDate(document.header.osh_voucher_date, 'header.osh_voucher_date');
        return document.details
            .filter((detail) => !detail.osl_is_deleted)
            .map((detail) => {
            const conversionFactor = detail.osl_conv_factor ?? 1;
            const qty = this.roundQuantity((detail.osl_base_qty ?? (detail.osl_qty ?? 0) * conversionFactor) * direction);
            const freeQty = this.roundQuantity((detail.osl_free_qty ?? 0) * conversionFactor * direction);
            const inwardValue = this.roundAmount((detail.osl_stock_value ?? (detail.osl_cost_rate ?? 0) * (detail.osl_qty ?? 0)) *
                direction);
            const inwardValueWot = this.roundAmount((detail.osl_stock_value_wot ?? (detail.osl_cost_rate_wot ?? 0) * (detail.osl_qty ?? 0)) *
                direction);
            const legacyInwardValue = this.roundAmount((detail.osl_stock_value_wot ?? (detail.osl_cost_rate_wot ?? 0) * (detail.osl_qty ?? 0)) *
                direction);
            const legacyInwardValueWot = this.roundAmount((detail.osl_stock_value ?? (detail.osl_cost_rate ?? 0) * (detail.osl_qty ?? 0)) *
                direction);
            return {
                ...
                stockBucket: this.normalizeStockBucket(item_stock_types_1.ItemStockBucket.SALEABLE, 'movement.stockBucket'),
                movementType: item_stock_types_1.StockTxnType.OPENING,
                qty,
                freeQty,
                inwardValue,
                inwardValueWot,
    ...
    async resolveBatchIdsForDocument(tx, document, actorUserId, now, persistResolvedBatchIds = false) {
        const resolutionContext = {
            batchScopeEntriesByKey: new Map(),
            batchMasterNosByItemKey: new Map(),
            batchMastersById: new Map(),
        };
        const details = [];
        for (const detail of document.details) {
            ...
            const resolvedBatch = await this.resolveOrCreateBatchIdentity(tx, detail, actorUserId, now, resolutionContext);
    ...
    async resolveOrCreateBatchIdentity(tx, detail, actorUserId, now, resolutionContext) {
        ...
        if (requestedBatchNoKey) {
            ...
            if (scopedBatch && detail.osl_batch_id && scopedBatch.batchId !== detail.osl_batch_id) {
                throw new common_1.ConflictException(`Batch no "${requestedBatchNo}" already exists for the selected company, branch, godown, item, and unit`);
            }
        ...
        if (detail.osl_batch_id) {
            ...
            if (scopedBatch) {
                if (requestedBatchNoKey && scopedBatch.batchNoKey !== requestedBatchNoKey) {
                    throw new common_1.ConflictException(`Batch id "${detail.osl_batch_id}" is already linked to batch no "${scopedBatch.batchNo}" for the selected company, branch, godown, item, and unit`);
                }
                return scopedBatch;
            }
            const existingBatchMaster = await this.getBatchMasterById(tx, detail.osl_batch_id, resolutionContext);
            if (!existingBatchMaster) {
                if (!requestedBatchNo) {
                    throw new common_1.BadRequestException(`Batch id "${detail.osl_batch_id}" does not exist for the requested stock movement`);
                }
            ...
            if (!resolvedBatchNo) {
                throw new common_1.BadRequestException(`Batch id "${detail.osl_batch_id}" does not have a valid saved batch no`);
            }
            if (requestedBatchNoKey &&
                this.normalizeBatchNoKey(resolvedBatchNo) !== requestedBatchNoKey) {
                throw new common_1.ConflictException(`Batch id "${detail.osl_batch_id}" is already linked to batch no "${resolvedBatchNo}"`);
            }
            const resolvedIdentity = {
        ...
        if (!batchNo) {
            ...
            batchNo = (0, batch_number_util_1.generateNextBatchNo)(existingBatchNos, { prefix: configuredPrefix });
        }
        ...
        const physicalQty = this.roundQuantity((detail.osl_base_qty ?? (detail.osl_qty ?? 0) * (detail.osl_conv_factor ?? 1)) +
            (detail.osl_free_qty ?? 0) * (detail.osl_conv_factor ?? 1));
        const lastPurchaseRateWot = physicalQty > 0 ? this.roundRate((detail.osl_stock_value_wot ?? 0) / physicalQty) : 0;
        ...
        const batchMasterNosKey = this.buildBatchMasterNosKey(detail.osl_company_id, detail.osl_item_id);
    ...
    buildBatchScopeReference(detail) {
        return {
            companyId: detail.osl_company_id,
            branchId: detail.osl_branch_id,
            godownId: detail.osl_godown_id,
    ...
    buildBatchScopeCacheKey(scope) {
        return [scope.companyId, scope.branchId, scope.godownId, scope.itemId, scope.unitId].join('|');
    }
    buildBatchMasterNosKey(companyId, itemId) {
        return [companyId, itemId].join('|');
    }
    async getBatchScopeEntries(tx, scope, resolutionContext) {
    ...
    registerBatchScopeEntry(resolutionContext, scope, batch) {
        const cacheKey = this.buildBatchScopeCacheKey(scope);
        ...
        const existingEntry = entries.find((entry) => entry.batchId === batch.batchId || entry.batchNoKey === batchNoKey);
        if (existingEntry) {
    ...
    async getBatchMasterNosForItem(tx, detail, resolutionContext) {
    ...
    async getBatchMasterById(tx, batchId, resolutionContext) {
    ...
    async getConfiguredBatchPrefix(tx, resolutionContext) {
    ...
    normalizeBatchNo(value) {
        const normalizedValue = value?.trim();
        return normalizedValue ? normalizedValue : null;
    }
    normalizeBatchNoKey(value) {
        return value.trim().toLowerCase();
    }
    async applyStockMovements(tx, movements, actorUserId, now) {
        const stockBalances = new Map();
        const batchStocks = new Map();
        for (const movement of movements) {
            ...
            if (this.isBatchManagedTrackingType(normalizedMovement.trackingType)) {
                if (!normalizedMovement.batchId) {
                    throw new common_1.BadRequestException('Batch-managed stock movement requires batch id for BATCH or MRP tracking');
                }
                ...
                const summaryRow = await this.rollupBatchToItemSummary(tx, normalizedMovement, actorUserId, now);
                if (summaryRow) {
    ...
    async applySummaryMovement(tx, movement, actorUserId, now) {
        const where = {
        ...
        const nextState = this.applyMovementToState(existing ? this.toStateFromStockBalance(existing) : this.createEmptyState(), movement);
    ...
    async applyBatchMovement(tx, movement, actorUserId, now) {
        const where = {
        ...
        const nextState = this.applyMovementToState(existing ? this.toStateFromBatchStock(existing) : this.createEmptyState(), movement);
    ...
    isReverseOpeningMovement(movement) {
        return (movement.movementType === item_stock_types_1.StockTxnType.OPENING &&
            (movement.qty < 0 || movement.freeQty < 0 || movement.inwardValue < 0));
    }
    async rollupBatchToItemSummary(tx, movement, actorUserId, now) {
        const where = {
            isbAccYear_isbCompanyId_isbBranchId_isbGodownId_isbItemId_isbUnitId_isbStockBucket: {
                isbAccYear: movement.accYear,
                isbCompanyId: movement.companyId,
        ...
        const nextState = this.mergeSummaryWotValuation(this.aggregateBatchStates(batchRows), this.resolveBatchSummaryWotState(existingSummary, movement));
        const savedRow = await tx.itemStockBalance.upsert({
            where,
            create: this.buildStockBalanceCreateInput(movement, nextState, actorUserId, now),
            update: this.buildStockBalanceUpdateInput(movement, nextState, actorUserId, now),
    ...
    aggregateBatchStates(rows) {
        ...
        for (const row of rows) {
            ...
            state.openingFreeQty = this.roundQuantity(state.openingFreeQty + this.toNumber(row.ibsOpeningFreeQty));
            ...
            state.openingValue = this.roundAmount(state.openingValue + this.toNumber(row.ibsOpeningValue));
    ...
    applyMovementToState(currentState, movement) {
        ...
        const inwardValue = this.resolveMovementValue(movement, movement.inwardValue, currentState.openingValue, currentState.stockValue, movement.legacyInwardValue);
        const inwardValueWot = this.resolveMovementValue(movement, movement.inwardValueWot, currentState.openingValueWot, currentState.stockValueWot, movement.legacyInwardValueWot, true);
        if (category === 'OPENING') {
        ...
        if (physicalQty > physicalClosingBefore) {
            throw new common_1.BadRequestException(`Insufficient stock for item ${movement.itemId}. Required ${physicalQty}, available ${physicalClosingBefore}`);
        }
    ...
    recalculateState(state) {
        ...
        nextState.closingQty = this.sanitizeQty(nextState.openingQty + nextState.inQty - nextState.outQty);
        nextState.freeClosingQty = this.sanitizeQty(nextState.openingFreeQty + nextState.freeInQty - nextState.freeOutQty);
        if (nextState.reservedQty > nextState.closingQty) {
            throw new common_1.BadRequestException(`Reserved qty ${nextState.reservedQty} cannot exceed closing qty ${nextState.closingQty}`);
        }
    ...
    getMovementCategory(movementType) {
        switch (movementType) {
            case item_stock_types_1.StockTxnType.OPENING:
                return 'OPENING';
            case item_stock_types_1.StockTxnType.PURCHASE:
            case item_stock_types_1.StockTxnType.SALES_RETURN:
            case item_stock_types_1.StockTxnType.TRANSFER_IN:
            case item_stock_types_1.StockTxnType.ADJUSTMENT_IN:
            case item_stock_types_1.StockTxnType.PRODUCTION_IN:
                return 'INWARD';
            case item_stock_types_1.StockTxnType.PURCHASE_RETURN:
            case item_stock_types_1.StockTxnType.SALE:
            case item_stock_types_1.StockTxnType.TRANSFER_OUT:
            case item_stock_types_1.StockTxnType.ADJUSTMENT_OUT:
            case item_stock_types_1.StockTxnType.CONSUMPTION:
            case item_stock_types_1.StockTxnType.DAMAGE:
            case item_stock_types_1.StockTxnType.EXPIRED:
                return 'OUTWARD';
            default:
                return 'OUTWARD';
        }
    ...
    resolveMovementValue(movement, primaryValue, currentOpeningValue, currentStockValue, fallbackValue, fallbackToZero = false) {
        const inwardValue = this.roundAmount(primaryValue);
        if (movement.movementType !== item_stock_types_1.StockTxnType.OPENING || inwardValue >= 0) {
            return inwardValue;
        }
    ...
    mergeSummaryWotValuation(baseState, wotState) {
    ...
    resolveBatchSummaryWotState(existingSummary, movement) {
        if (existingSummary) {
    ...
    buildStockBalanceCreateInput(movement, state, actorUserId, now) {
        return {
            isbAccYear: movement.accYear,
            isbCompanyId: movement.companyId,
            isbBranchId: movement.branchId,
    ...
    buildStockBalanceUpdateInput(movement, state, actorUserId, now) {
        return {
            isbTrackingType: movement.trackingType,
            isbOpeningQty: state.openingQty,
            isbInQty: state.inQty,
    ...
    buildBatchStockCreateInput(movement, state, actorUserId, now) {
        return {
            ibsAccYear: movement.accYear,
            ibsCompanyId: movement.companyId,
            ibsBranchId: movement.branchId,
    ...
    buildBatchStockUpdateInput(movement, state, actorUserId, now) {
        return {
            ibsUnitId: movement.unitId,
            ibsBatchNo: movement.batchNo ?? null,
            ibsMfgDate: movement.mfgDate ?? null,
    ...
    resolveLedgerAuditAction(existingLedgerRows, header) {
        if (header.osh_is_deleted) {
            return 'cancel';
        }
        return existingLedgerRows.length > 0 ? 'update' : 'New';
    }
    async logOpeningStockPostingChange(tx, input) {
    ...
    buildDocumentDisplayName(header) {
        return (this.firstNonEmptyText(header.osh_ref_no, header.osh_voucher_no, header.avh_voucher_refno, header.avh_voucher_id) ?? header.avh_voucher_id);
    }
    buildDetailDisplayName(detail) {
        const parts = [
    ...
    buildMovementDisplayName(movement) {
        const parts = [
    ...
    buildStockBalanceAuditPk(row, movement) {
        return row.isbId || this.buildSummaryScopeKey(movement);
    }
    buildBatchStockAuditPk(row, movement) {
    ...
    assertValidBatchStockWrite(movement, state) {
        ...
        if (this.roundQuantity(state.closingQty) !== expectedClosingQty) {
            throw new common_1.BadRequestException('itemBatchStock.ibsClosingQty must equal ibsOpeningQty + ibsInQty - ibsOutQty');
        }
        const expectedFreeClosingQty = this.roundQuantity(state.openingFreeQty + state.freeInQty - state.freeOutQty);
        if (this.roundQuantity(state.freeClosingQty) !== expectedFreeClosingQty) {
            throw new common_1.BadRequestException('itemBatchStock.ibsFreeClosingQty must equal ibsOpeningFreeQty + ibsFreeInQty - ibsFreeOutQty');
        }
        ...
        if (this.roundQuantity(state.availableQty) !== expectedAvailableQty) {
            throw new common_1.BadRequestException('itemBatchStock.ibsAvailableQty must equal ibsClosingQty - ibsReservedQty');
        }
    }
    assertValidBatchStockDates(mfgDate, expiryDate) {
        if (mfgDate && expiryDate && expiryDate.getTime() < mfgDate.getTime()) {
            throw new common_1.BadRequestException('itemBatchStock.ibsExpiryDate must be greater than or equal to itemBatchStock.ibsMfgDate');
        }
    }
    assertNonNegativeQuantity(value, field) {
        if (this.roundQuantity(value) < 0) {
            throw new common_1.BadRequestException(`${field} must be greater than or equal to 0`);
        }
    }
    assertNonNegativeRate(value, field) {
        if (this.roundRate(value) < 0) {
            throw new common_1.BadRequestException(`${field} must be greater than or equal to 0`);
        }
    }
    assertNonNegativeAmount(value, field) {
    ...
    normalizeBatchStatus(value, field) {
        const normalized = value?.trim().toUpperCase();
        const candidate = normalized;
        if (!candidate || !ITEM_BATCH_STATUS_SET.has(candidate)) {
            throw new common_1.BadRequestException(`${field} must be one of: ${ITEM_BATCH_STATUS_VALUES.join(', ')}`);
        }
        return candidate;
    }
    normalizeStockBucket(value, field) {
        const normalized = value?.trim().toUpperCase();
        const candidate = normalized;
        if (!candidate || !ITEM_STOCK_BUCKET_SET.has(candidate)) {
            throw new common_1.BadRequestException(`${field} must be one of: ${ITEM_STOCK_BUCKET_VALUES.join(', ')}`);
        }
        return candidate;
    }
    toStockTrackingType(value) {
        const normalized = value?.trim().toUpperCase();
        const trackingType = (normalized === 'BATCH' ? 'BATCH' : normalized === 'MRP' ? 'MRP' : 'NONE');
        return trackingType;
    }
    toBalanceTrackingType(value) {
        const normalized = value?.trim().toUpperCase();
        const trackingType = (normalized === 'BATCH' ? 'BATCH' : normalized === 'MRP' ? 'MRP' : 'NONE');
        return trackingType;
    }
    isBatchManagedTrackingType(trackingType) {
    ...
    parseRequiredDate(value, field) {
        ...
        if (Number.isNaN(parsed.getTime())) {
            throw new common_1.BadRequestException(`${field} must be a valid ISO date string`);
        }
        return parsed;
    }
    ...
    handleWriteError(error) {
        if ((0, module_service_utils_1.isUniqueConstraintError)(error)) {
            throw new common_1.ConflictException('Duplicate item stock ledger or item stock balance rows are not allowed');
        }
        if ((0, module_service_utils_1.isForeignKeyConstraintError)(error)) {
            throw new common_1.BadRequestException('One or more referenced ids do not exist for the requested stock posting');
        }
    }
};
exports.ItemStockLedgerService = ItemStockLedgerService;
exports.ItemStockLedgerService = ItemStockLedgerService = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [prisma_service_1.PrismaService,
        request_context_service_1.RequestContextService,
        audit_log_service_1.AuditLogService])
], ItemStockLedgerService);
//# sourceMappingURL=item-stock-ledger.service.js.map