...
exports.VoucherPostingService = void 0;
const common_1 = require("@nestjs/common");
const client_1 = require("@prisma/client");
const prisma_service_1 = require("../../database/prisma/prisma.service");
const voucher_sequence_helper_1 = require("../Sequence/voucher-sequence.helper");
const ledger_map_helper_1 = require("../../modules/accountsModule/ledgerRole/ledger-map.helper");
const DEVICE_TYPES = new Set(['PC', 'WEB', 'MOBILE', 'POS', 'DESKTOP']);
const REVERSAL_VOUCHER_TYPE_CODE = 'Rev';
let VoucherPostingService = class VoucherPostingService {
    prisma;
    constructor(prisma) {
        this.prisma = prisma;
    }
    ...
    async restateLegs(tx, doc, legs, ledgerByLeg, voucherId) {
      ...
      SELECT avh_voucher_no, avh_voucher_slno, avh_voucher_refno
       ...
       WHERE avh_voucher_id = ${voucherId}::uuid
         ...
         FOR UPDATE`;
        if (!kept) {
            throw new Error(`${describe(doc.header)}: voucher ${voucherId} is not in DRAFT — retireForRestate must run first`);
        }
    ...
    async postIntoDraft(tx, doc, legs, ledgerByLeg, voucherId) {
        const accYear = doc.header.accYear;
      ...
      SELECT avh_voucher_status
       ...
       WHERE avh_voucher_id = ${voucherId}::uuid
        ...
        if (!draft || draft.avh_voucher_status !== 'DRAFT') {
            throw new Error(`${describe(doc.header)}: voucher ${voucherId} is ${draft?.avh_voucher_status ?? 'missing'}, not a DRAFT`);
        }
        const slno = await (0, voucher_sequence_helper_1.allocateVoucherSlno)(tx, doc.header.companyId, accYear);
    ...
    async reversalVoucherTypeId(tx) {
      ...
      SELECT vchr_type_id FROM accounts.acc_voucher_types
       ...
       LIMIT 1`;
        if (!row) {
            throw new Error(`Voucher type '${REVERSAL_VOUCHER_TYPE_CODE}' is missing — apply migration 20260925150000_reversal_voucher_type`);
        }
    ...
    assertBalanced(header, legs) {
        ...
        if (round2(dr) !== round2(cr)) {
            ...
            throw new Error(`${describe(header)} does not balance: ` +
                `debits ${round2(dr)}, credits ${round2(cr)}, ` +
                `${diff > 0 ? 'debit' : 'credit'} heavy by ${Math.abs(diff)}`);
        }
    }
};
exports.VoucherPostingService = VoucherPostingService;
exports.VoucherPostingService = VoucherPostingService = __decorate([
...
//# sourceMappingURL=voucher-posting.service.js.map