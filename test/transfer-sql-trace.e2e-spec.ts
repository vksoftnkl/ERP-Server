...
function classify(sql: string): { verb: string; target: string } {
  ...
  const tables = [...s.matchAll(/\b(?:FROM|INTO|UPDATE|JOIN)\s+"?([a-z_]+)"?\."?([a-z_]+)"?/gi)].map((m) => `${m[1]}.${m[2]}`);
...
describe('TRACE /stock/transfer — statements, tables, functions', () => {
  ...
  beforeAll(async () => {
    const claims: AccessTokenPayload = {
      sub: ACTOR, user_name: 'tester1', sid: 'e2e-trf-trace', user_type: 'SUPER ADMIN',
    ...
    const svc = app.get(PrismaService) as unknown as { $on: (e: string, cb: (ev: { query: string }) => void) => void };
    svc.$on('query', (ev) => { if (capture) capture.push(classify(ev.query)); });
  }, 120_000);
  ...
  async function trace<T>(label: string, run: () => Promise<T>): Promise<T> {
    ...
    steps.forEach((s, i) => out.push(`  ${String(i + 1).padStart(2)}. ${s.verb.padEnd(11)} ${s.target}`));
    out.push('');
    return result!;
  }

  const scopeQuery = { accYear: ACC_YEAR, companyId: SCOPE.companyId, branchId: SCOPE.branchId };
  let lot: { lotId: string; itemId: string; uomId: string; batchNo: string | null; expiryDate: string | null };
  let outId = '';
  let interId = '';
  ...
  beforeAll(async () => {
    ...
    lot = {
      lotId: row.lot, itemId: row.item, uomId: row.uom,
  ...
  function outBody(interBranch = false) {
    return {
      header: {
        ...
        deviceId: SCOPE.deviceId, docDate: DOC_DATE, fromGodownId: SRC_GODOWN,
        toGodownId: interBranch ? OTHER_GODOWN : DST_GODOWN,
        ...(interBranch ? { toBranchId: OTHER_BRANCH } : {}),
        userId: ACTOR, remarks: 'E2E-TRF sql trace',
      },
      lines: [{
  ...
  it('traces every route that runs', async () => {
    ...
    await trace('GET /transfer — load one (with transit rows)', () =>
      http.get(T).set('Authorization', BEARER).query({ ...scopeQuery, svhId: outId }));

    await trace('GET /transfer/validate — the preflight', () =>
      http.get(`${T}/validate`).set('Authorization', BEARER).query({ ...scopeQuery, svhId: outId }));

    await trace('POST /transfer/despatch — refused by the opening guard (422)', () =>
      http.post(`${T}/despatch`).set('Authorization', BEARER)
        .send({ svhId: outId, ...scopeQuery, userId: ACTOR, lrNo: 'LR-1', vehicleNo: 'TN-99', expectedOn: DOC_DATE }));

    await trace('POST /transfer/cancel — refused, a DRAFT is deleted not cancelled (409)', () =>
      http.post(`${T}/cancel`).set('Authorization', BEARER)
    ...
    await trace('GET /transfer/receive/inbound — the receiving branch worklist', () =>
      http.get(`${RCV}/inbound`).set('Authorization', BEARER)
    ...
    await trace('GET /transfer/receive/prefill — against a DRAFT despatch (409)', () =>
      http.get(`${RCV}/prefill`).set('Authorization', BEARER)
        .query({ companyId: SCOPE.companyId, branchId: OTHER_BRANCH, accYear: ACC_YEAR, outVoucherId: interId }));

    await trace('POST /transfer/receive — the TRANSFER_IN half (409, nothing despatched)', () =>
      http.post(RCV).set('Authorization', BEARER).send({
        header: {
          ...
          linkSrcDocId: interId, linkSrcAccYear: ACC_YEAR,
        },
        lines: [{
    ...
    await trace('DELETE /transfer — soft delete a DRAFT', () =>
      http.delete(T).set('Authorization', BEARER).query({ svhId: outId, ...scopeQuery }));
  }, 180_000);
    ...
    await trace('StockTransferService.despatch — opening guard bypassed', async () => {
      try {
        await service.despatch(bypassed, {
          svhId: interId, accYear: ACC_YEAR, companyId: SCOPE.companyId,
          branchId: SCOPE.branchId, userId: ACTOR,
          lrNo: 'LR-2', vehicleNo: 'TN-88', expectedOn: DOC_DATE,
        });
      } catch (e) {
        caught = e;
      }
