...
/**
 * Stock adjustment reasons + the GST UQC unit list.
 *
 * Both are reference data the stock modules select from, so they have to exist in
 * every environment before the first adjustment is entered. Written as upserts
 * keyed on the business code, which is what makes the seed safe to re-run.
 */
export const stockAdjustReasonsSeed: TsSeed = {
  name: 'stock-adjust-reasons',
  version: '1',
  mode: 'always',
  description: 'stock.stock_adj_reasons + fixed GST UQC units',
  async run(prisma: PrismaClient): Promise<void> {
    for (const reason of STOCK_ADJUST_REASONS) {
    ...
    for (const unit of UNIT_UQC_LIST) {
      // itemGstUnitCode carries no unique index, so this is a find-then-write
