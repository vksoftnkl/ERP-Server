...
import { allocateVoucherNumber, allocateVoucherSlno } from '../Sequence/voucher-sequence.helper';
import {
  requireRoleLedgers,
  roleLedgerKey,
  type RoleLedgerRequest,
} from '../../modules/accountsModule/ledgerRole/ledger-map.helper';
import type {
  VoucherHeaderInput,
  VoucherLeg,
  VoucherLegSource,
  VoucherPostingResult,
} from './voucher-leg.types';

/** `ck_avh_device_type` accepts these; anything else becomes NULL. */
const DEVICE_TYPES = new Set(['PC', 'WEB', 'MOBILE', 'POS', 'DESKTOP']);

/**
 * The ONE posting routine (voucher_register.md §7): every document that reaches
 * the ledger — sale bill, return, challan, the Voucher Register's eight types,
 * and the Receipt once it moves onto it — writes its header and legs HERE.
 * Lifted from src/modules/sales/posting/sales-posting.service.ts unchanged in
 * behaviour; the sales module keeps its `SalesPostingService` name as an alias.
 *
...
/** The voucher type every sales cancel's mirror is numbered in (notes 49 D3). */
const REVERSAL_VOUCHER_TYPE_CODE = 'Rev';
@Injectable()
export class VoucherPostingService {
  ...
  async postLegs(
    ...
    doc: VoucherLegSource,
  ): Promise<VoucherPostingResult> {
    const legs = doc.legs.filter((l) => round2(l.amount) !== 0);
    if (legs.length === 0) {
      throw new Error(`${describe(doc.header)} produced no voucher legs`);
    }
    ...
    if (doc.header.draftVoucherId) {
      return this.postIntoDraft(tx, doc, legs, ledgerByLeg, doc.header.draftVoucherId);
    }
    ...
    const [header] = await tx.$queryRaw<{ avh_voucher_id: string }[]>`
      INSERT INTO accounts.acc_voucher_header (
        ...
        ${doc.header.srcModule ?? null}, ${doc.header.srcDocType ?? null}, ${doc.header.srcDocId ?? null}::uuid,
        ...
        ${doc.header.partyId ?? null}::uuid, ${doc.header.remarks ?? null},
  ...
  private async restateLegs(
    ...
    if (!kept) {
      throw new Error(
        `${describe(doc.header)}: voucher ${voucherId} is not in DRAFT — retireForRestate must run first`,
      );
    }
    ...
    await tx.$executeRaw`
      UPDATE accounts.acc_voucher_header
         SET avh_voucher_date = ${doc.header.voucherDate}::date,
             ...
             avh_party_id     = ${doc.header.partyId ?? null}::uuid,
  ...
  /**
   * The Voucher Register's way in (voucher_register.md §8): `/create` wrote a
   * DRAFT header (payload in avh_draft_lines, no legs, no number — ck_avh_no
   * lets a DRAFT go unnumbered); `/post` derives the legs and writes them into
   * THAT header, numbering it now so a deleted draft never burns a number.
   */
  private async postIntoDraft(
    tx: Prisma.TransactionClient,
    doc: VoucherLegSource,
    legs: VoucherLeg[],
    ledgerByLeg: (string | null)[],
    ...
    const accYear = doc.header.accYear;
    const [draft] = await tx.$queryRaw<{ avh_voucher_status: string }[]>`
      SELECT avh_voucher_status
        FROM accounts.acc_voucher_header
       WHERE avh_voucher_id = ${voucherId}::uuid
         AND avh_acc_year   = ${accYear}::char(9)
         AND avh_is_deleted = false
         FOR UPDATE`;
    if (!draft || draft.avh_voucher_status !== 'DRAFT') {
      throw new Error(
        `${describe(doc.header)}: voucher ${voucherId} is ${draft?.avh_voucher_status ?? 'missing'}, not a DRAFT`,
      );
    }

    const slno = await allocateVoucherSlno(tx, doc.header.companyId, accYear);
    ...
    // Legs first, status last: ck_avh_balanced binds a POSTED header only, and
    // tr_av_refresh_totals fills the totals as the legs land.
    await this.insertLegs(tx, doc, legs, ledgerByLeg, voucherId, number);

    const postedOn = new Date();
    await tx.$executeRaw`
      UPDATE accounts.acc_voucher_header
         SET avh_voucher_type_id = ${doc.header.voucherTypeId}::int,
             avh_voucher_no      = ${number.lastNo}::bigint,
             avh_voucher_slno    = ${slno}::bigint,
             avh_voucher_refno   = ${number.refno},
             avh_voucher_date    = ${doc.header.voucherDate}::date,
             avh_usr_refno       = ${doc.header.usrRefno ?? null},
             avh_doc_refno       = ${doc.header.docRefno ?? null},
             avh_doc_date        = ${doc.header.docDate ?? doc.header.voucherDate}::date,
             avh_doc_amount      = ${money(doc.header.docAmount)}::numeric,
  ...
  private async insertLegs(
    ...
    const values = legs.map(
      (leg, i) => Prisma.sql`(
        ...
        ${doc.header.docDate ?? doc.header.voucherDate}::date,
        ${leg.drCr}::bpchar, ${ledgerByLeg[i]}::uuid, ${leg.oppLedgerId ?? null}::uuid,
    ...
    await tx.$executeRaw`
      INSERT INTO accounts.acc_vouchers (
        ...
        av_doc_date,
        av_dr_cr, av_ledger_id, av_opp_ledger_id,
        av_amount,
  ...
  /**
   * Cancel: write the MIRROR voucher and mark the original.
   *
   ...
   *    rows), so "same number" is a 23505. And it is numbered in the
   *    REVERSAL type ('Rev', rev00001…), not the original's: drawing on the
   *    original's series burned a sale-bill number on every cancel and left a
   *    gap in the GST invoice series (notes 49, D3);
  ...
  async reverseLegs(
    ...
    // 2 · a number of its own — in the REVERSAL series, never the original's
    //     (notes 49 D3: a bill cancel drawing on the bill series left a gap in
    //     the GST invoice numbers). On the original's date.
    const reversalTypeId = await this.reversalVoucherTypeId(tx);
    const number = await allocateVoucherNumber(tx, {
      vchrTypeId: reversalTypeId,
    ...
    const [mirror] = await tx.$queryRaw<{ avh_voucher_id: string }[]>`
      ...
      SELECT o.avh_company_id, o.avh_branch_id, o.avh_tenant_id, o.avh_acc_year,
             ${reversalTypeId}::int, ${number.lastNo}::bigint, ${slno}::bigint, ${number.refno},
    ...
    const legCount = await tx.$executeRaw`
      ...
      SELECT ${mirror.avh_voucher_id}::uuid, o.av_company_id, o.av_branch_id,
             ...
             ${reversalTypeId}::int, ${number.lastNo}::bigint, o.av_row_no, o.av_voucher_date,
  ...
  /**
   * accounts.acc_voucher_types 'Rev' (migration 20260925150000). By code, not
   * id: the id is a sequence value and differs between databases.
   */
  private async reversalVoucherTypeId(tx: Prisma.TransactionClient): Promise<number> {
    const [row] = await tx.$queryRaw<{ vchr_type_id: number }[]>`
      SELECT vchr_type_id FROM accounts.acc_voucher_types
       WHERE vchr_type_code = ${REVERSAL_VOUCHER_TYPE_CODE} AND vchr_is_active = true
       LIMIT 1`;
    if (!row) {
      throw new Error(
        `Voucher type '${REVERSAL_VOUCHER_TYPE_CODE}' is missing — apply migration 20260925150000_reversal_voucher_type`,
      );
    }
    ...
    const resolved =
      requests.length === 0
        ...
        : await requireRoleLedgers(tx, requests, {
            ...
            where: header.srcDocType ?? header.docLabel ?? 'VOUCHER',
            // The sales documents keep the text every client already knows.
            message: `${header.docLabel ?? 'Bill'} cannot be posted`,
          });
  ...
  private assertBalanced(header: VoucherHeaderInput, legs: VoucherLeg[]): void {
    const { dr, cr } = sumSides(legs);
    if (round2(dr) !== round2(cr)) {
      const diff = round2(dr - cr);
      throw new Error(
        `${describe(header)} does not balance: ` +
...
/** "SALE_BILL <id>" for a document's voucher, "Purchase (Accounting) <refno>" for the register's. */
function describe(header: VoucherHeaderInput): string {
  const what = header.srcDocType ?? header.docLabel ?? 'Voucher';
  const which = header.srcDocId ?? header.docRefno ?? '';
  return `${what} ${which}`.trim();
}

function sumSides(legs: VoucherLeg[]): { dr: number; cr: number } {
  let dr = 0;
  let cr = 0;
  for (const leg of legs) {
    if (leg.drCr === 'DR') {
