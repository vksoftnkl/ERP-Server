...
/**
 * One requested settlement of the invoice against a credit. The sale bill's
 * `BillAdjustmentInput` satisfies it; the Voucher Register hands the same
 * shape from its own DTO. Lifted from src/modules/sales/bill with the helper.
 */
export interface BillAdjustmentInput {
  ...
  amount: number;
  remarks?: string | null;
  /** Echoes the client may send; ignored — the server reads them off the credit. */
  billType?: string;
  adjType?: string;
  settlementMode?: string;
}
type BillErrorDetail = { field: string; message: string };
...
export async function syncBillAdjustments(
  ...
  adjustments: BillAdjustmentInput[] | undefined,
  actor: string,
  now: Date,
...
function normalizeRequest(adjustments: BillAdjustmentInput[]): BillAdjustmentInput[] {
  const merged = new Map<string, BillAdjustmentInput>();
...
function matches(live: PostedAdjustment[], requested: BillAdjustmentInput[]): boolean {
