...
describe('ItemCategoryExceptionFilter', () => {
  let filter: ItemCategoryExceptionFilter;
  let status: jest.Mock;
  let json: jest.Mock;
  let response: Response;

  beforeEach(() => {
    filter = new ItemCategoryExceptionFilter();
    json = jest.fn();
  ...
  it('passes through standardized error payloads', () => {
    const exception = new NotFoundException({
      ...
      errors: [{ field: 'category_id', message: 'Not found' }],
    });
  ...
  it('maps validation payload to standard error format', () => {
    const exception = new BadRequestException({
      message: ['category_name should not be empty'],
    });
