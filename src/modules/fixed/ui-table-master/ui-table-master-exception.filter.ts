...
@Catch()
export class UiTableMasterExceptionFilter implements ExceptionFilter {
    ...
    if (exception instanceof HttpException) {
      ...
      if (this.isUiTableMasterErrorResponse(rawResponse)) {
  ...
  private isUiTableMasterErrorResponse(value: unknown): value is UiTableMasterErrorResponse {
    if (typeof value !== 'object' || value === null) {
      return false;
    }

    const candidate = value as Partial<UiTableMasterErrorResponse>;
    return (
  ...
  private mapValidationPayload(payload: ValidationExceptionPayload): UiTableMasterErrorResponse {
    ...
    const errors: UiTableMasterErrorDetail[] = messages.map((message) => ({
  ...
  private inferFieldName(message: string): string {
    const fieldMatch = message.match(/\b(uiTbl[A-Za-z0-9]+)\b/);
    if (fieldMatch) {
      return fieldMatch[1];
    }
