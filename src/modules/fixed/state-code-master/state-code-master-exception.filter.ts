...
@Catch()
export class StateCodeMasterExceptionFilter implements ExceptionFilter {
    ...
    if (exception instanceof HttpException) {
      ...
      if (this.isStateCodeMasterErrorResponse(rawResponse)) {
  ...
  private isStateCodeMasterErrorResponse(value: unknown): value is StateCodeMasterErrorResponse {
    if (typeof value !== 'object' || value === null) {
      return false;
    }

    const candidate = value as Partial<StateCodeMasterErrorResponse>;
    return (
  ...
  private mapValidationPayload(payload: ValidationExceptionPayload): StateCodeMasterErrorResponse {
    ...
    const errors: StateCodeMasterErrorDetail[] = messages.map((message) => ({
  ...
  private inferFieldName(message: string): string {
    const fieldMatch = message.match(/\b(stateCode|stateName|tinCode|isActive|stateUt)\b/);
    if (fieldMatch) {
      return fieldMatch[1];
    }
