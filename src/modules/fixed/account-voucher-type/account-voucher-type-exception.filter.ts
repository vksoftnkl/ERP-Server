...
@Catch()
export class AccountVoucherTypeExceptionFilter implements ExceptionFilter {
    ...
    if (exception instanceof HttpException) {
      ...
      if (this.isAccountVoucherTypeErrorResponse(rawResponse)) {
  ...
  private isAccountVoucherTypeErrorResponse(
    value: unknown,
  ): value is AccountVoucherTypeErrorResponse {
    if (typeof value !== 'object' || value === null) {
      return false;
    }

    const candidate = value as Partial<AccountVoucherTypeErrorResponse>;
    return (
  ...
  private mapValidationPayload(
    ...
    const errors: AccountVoucherTypeErrorDetail[] = messages.map((message) => ({
  ...
  private inferFieldName(message: string): string {
    const fieldMatch = message.match(/\b(avt[A-Za-z0-9]+)\b/);
    if (fieldMatch) {
      return fieldMatch[1];
    }
