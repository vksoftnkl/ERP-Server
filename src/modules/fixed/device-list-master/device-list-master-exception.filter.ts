...
@Catch()
export class DeviceListMasterExceptionFilter implements ExceptionFilter {
    ...
    if (exception instanceof HttpException) {
      ...
      if (this.isDeviceListMasterErrorResponse(rawResponse)) {
  ...
  private isDeviceListMasterErrorResponse(value: unknown): value is DeviceListMasterErrorResponse {
    if (typeof value !== 'object' || value === null) {
      return false;
    }

    const candidate = value as Partial<DeviceListMasterErrorResponse>;
    return (
  ...
  private mapValidationPayload(payload: ValidationExceptionPayload): DeviceListMasterErrorResponse {
    ...
    const errors: DeviceListMasterErrorDetail[] = messages.map((message) => ({
  ...
  private inferFieldName(message: string): string {
    const fieldMatch = message.match(/\b(dev[A-Za-z0-9]+)\b/);
    if (fieldMatch) {
      return fieldMatch[1];
    }
