...
@Catch()
export class BankListExceptionFilter implements ExceptionFilter {
    ...
    if (exception instanceof HttpException) {
      ...
      if (this.isBankListErrorResponse(rawResponse)) {
  ...
  private isBankListErrorResponse(value: unknown): value is BankListErrorResponse {
    if (typeof value !== 'object' || value === null) {
      return false;
    }

    const candidate = value as Partial<BankListErrorResponse>;
    return (
  ...
  private mapValidationPayload(payload: ValidationExceptionPayload): BankListErrorResponse {
    ...
    const errors: BankListErrorDetail[] = messages.map((message) => ({
  ...
  private inferFieldName(message: string): string {
    const fieldMatch = message.match(/\b(bnk[A-Za-z0-9]+)\b/);
    if (fieldMatch) {
      return fieldMatch[1];
    }
