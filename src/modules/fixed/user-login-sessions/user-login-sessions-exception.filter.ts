...
@Catch()
export class UserLoginSessionsExceptionFilter implements ExceptionFilter {
    ...
    if (exception instanceof HttpException) {
      ...
      if (this.isUserLoginSessionsErrorResponse(rawResponse)) {
  ...
  private isUserLoginSessionsErrorResponse(
    value: unknown,
  ): value is UserLoginSessionsErrorResponse {
    if (typeof value !== 'object' || value === null) {
      return false;
    }

    const candidate = value as Partial<UserLoginSessionsErrorResponse>;
    return (
  ...
  private mapValidationPayload(
    ...
    const errors: UserLoginSessionsErrorDetail[] = messages.map((message) => ({
  ...
  private inferFieldName(message: string): string {
    const fieldMatch = message.match(/\b(uls[A-Za-z0-9]+)\b/);
    if (fieldMatch) {
      return fieldMatch[1];
    }
