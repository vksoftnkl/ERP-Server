...
export class UserLoginSessionsService {
  ...
  private async listFromConfiguredGridSql(
    ...
    limit: number,
    skip: number,
  ): Promise<{ items: UserLoginSessionsListItem[]; meta: UserLoginSessionsListMeta } | null> {
    const configuredGrid = await this.prisma.gridDetails.findFirst({
      where: {
        ...
        gridSql: {
          not: null,
          contains: USER_LOGIN_SESSIONS_TABLE_NAME,
          mode: 'insensitive',
        },
      },
    ...
    try {
      ...
      const countSql = `SELECT COUNT(*)::bigint AS total FROM (${baseSql}) AS user_login_sessions_grid`;
      const rowsSql = `SELECT * FROM (${baseSql}) AS user_login_sessions_grid LIMIT $1 OFFSET $2`;
      const [countResult, rows] = await Promise.all([
        this.prisma.$queryRawUnsafe<Array<{ total: bigint | number | string }>>(countSql),
        this.prisma.$queryRawUnsafe<UserLoginSessionsListItem[]>(rowsSql, limit, skip),
      ]);
  ...
  private validateConfiguredGridSql(sql: string): string {
    ...
    if (!/\buser_login_sessions\b/i.test(normalized)) {
      this.throwBadRequest('Invalid grid_sql configuration for user login sessions', [
        {
          field: 'grid_sql',
          message: 'Configured query must reference user_login_sessions table',
        },
      ]);
    }
