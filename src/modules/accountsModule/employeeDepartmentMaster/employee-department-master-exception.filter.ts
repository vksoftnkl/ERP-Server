...
@Catch()
export class EmployeeDepartmentMasterExceptionFilter implements ExceptionFilter {
  catch(exception: unknown, host: ArgumentsHost): void {
    const response = host.switchToHttp().getResponse<Response>();
    if (exception instanceof HttpException) {
      const statusCode = exception.getStatus();
      const rawResponse = exception.getResponse();
      if (this.isEmployeeDepartmentMasterErrorResponse(rawResponse)) {
  ...
  private isEmployeeDepartmentMasterErrorResponse(
    value: unknown,
  ): value is EmployeeDepartmentMasterErrorResponse {
    if (typeof value !== 'object' || value === null) {
      return false;
    }
    const candidate = value as Partial<EmployeeDepartmentMasterErrorResponse>;
    return (
  ...
  private mapValidationPayload(
    ...
    const messages = Array.isArray(payload.message)
      ...
      : payload.message
        ? [payload.message]
        : ['Validation failed'];
    const errors: EmployeeDepartmentMasterErrorDetail[] = messages.map((message) => ({
  ...
  private inferFieldName(message: string): string {
    const fieldMatch = message.match(/\b(edpt[A-Za-z0-9]+)\b/);
    if (fieldMatch) {
      return fieldMatch[1];
    }
    return 'request';
