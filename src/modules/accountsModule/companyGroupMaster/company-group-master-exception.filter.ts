...
@Catch()
export class CompanyGroupMasterExceptionFilter implements ExceptionFilter {
    ...
    if (exception instanceof HttpException) {
      ...
      if (this.isCompanyGroupMasterErrorResponse(rawResponse)) {
  ...
  private isCompanyGroupMasterErrorResponse(
    value: unknown,
  ): value is CompanyGroupMasterErrorResponse {
    if (typeof value !== 'object' || value === null) {
      return false;
    }

    const candidate = value as Partial<CompanyGroupMasterErrorResponse>;
    return (
  ...
  private mapValidationPayload(
    ...
    const errors: CompanyGroupMasterErrorDetail[] = messages.map((message) => ({
  ...
  private inferFieldName(message: string): string {
    const fieldMatch = message.match(/\b(cog[A-Za-z0-9]+)\b/);
    if (fieldMatch) {
      return fieldMatch[1];
    }
