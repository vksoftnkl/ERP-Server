...
@Catch()
export class BranchMasterExceptionFilter implements ExceptionFilter {
    ...
    if (exception instanceof HttpException) {
      ...
      if (this.isBranchMasterErrorResponse(rawResponse)) {
  ...
  private isBranchMasterErrorResponse(value: unknown): value is BranchMasterErrorResponse {
    if (typeof value !== 'object' || value === null) {
      return false;
    }

    const candidate = value as Partial<BranchMasterErrorResponse>;
    return (
  ...
  private mapValidationPayload(payload: ValidationExceptionPayload): BranchMasterErrorResponse {
    ...
    const errors: BranchMasterErrorDetail[] = messages.map((message) => ({
  ...
  private inferFieldName(message: string): string {
    const fieldMatch = message.match(/\b(br[A-Za-z0-9]+|compId)\b/);
    if (fieldMatch) {
      return fieldMatch[1];
    }
