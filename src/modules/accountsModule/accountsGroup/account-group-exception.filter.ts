...
@Catch()
export class AccountGroupExceptionFilter implements ExceptionFilter {
    ...
    if (exception instanceof HttpException) {
      ...
      if (this.isAccountGroupErrorResponse(rawResponse)) {
  ...
  private isAccountGroupErrorResponse(value: unknown): value is AccountGroupErrorResponse {
    if (typeof value !== 'object' || value === null) {
      return false;
    }

    const candidate = value as Partial<AccountGroupErrorResponse>;
    return (
  ...
  private mapValidationPayload(payload: ValidationExceptionPayload): AccountGroupErrorResponse {
    ...
    const errors: AccountGroupErrorDetail[] = messages.map((message) => ({
  ...
  private inferFieldName(message: string): string {
    const fieldMatch = message.match(/\b(accGroup[a-zA-Z0-9]+)\b/);
    if (fieldMatch) {
      return fieldMatch[1];
    }
