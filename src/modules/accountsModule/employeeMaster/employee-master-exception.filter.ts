...
@Catch()
export class EmployeeMasterExceptionFilter implements ExceptionFilter {
    ...
    if (exception instanceof HttpException) {
      ...
      if (this.isEmployeeMasterErrorResponse(rawResponse)) {
  ...
  private isEmployeeMasterErrorResponse(value: unknown): value is EmployeeMasterErrorResponse {
    if (typeof value !== 'object' || value === null) {
      return false;
    }

    const candidate = value as Partial<EmployeeMasterErrorResponse>;
    return (
  ...
  private mapValidationPayload(payload: ValidationExceptionPayload): EmployeeMasterErrorResponse {
    ...
    const errors: EmployeeMasterErrorDetail[] = messages.map((message) => ({
  ...
  private inferFieldName(message: string): string {
    const fieldMatch = message.match(/\b(emp[A-Za-z0-9]+)\b/);
    if (fieldMatch) {
      return fieldMatch[1];
    }
