...
@Catch()
export class GspProviderMasterExceptionFilter implements ExceptionFilter {
    ...
    if (exception instanceof HttpException) {
      ...
      if (this.isGspProviderMasterErrorResponse(rawResponse)) {
  ...
  private isGspProviderMasterErrorResponse(
    value: unknown,
  ): value is GspProviderMasterErrorResponse {
    if (typeof value !== 'object' || value === null) {
      return false;
    }

    const candidate = value as Partial<GspProviderMasterErrorResponse>;
    return (
  ...
  private mapValidationPayload(
    ...
    const errors: GspProviderMasterErrorDetail[] = messages.map((message) => ({
  ...
  private inferFieldName(message: string): string {
    const fieldMatch = message.match(/\b(gsp[A-Za-z0-9]+)\b/);
    if (fieldMatch) {
      return fieldMatch[1];
    }
