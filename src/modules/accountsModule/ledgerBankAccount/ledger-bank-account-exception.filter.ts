...
@Catch()
export class LedgerBankAccountExceptionFilter implements ExceptionFilter {
    ...
    if (exception instanceof HttpException) {
      ...
      if (this.isLedgerBankAccountErrorResponse(rawResponse)) {
  ...
  private isLedgerBankAccountErrorResponse(
    value: unknown,
  ): value is LedgerBankAccountErrorResponse {
    if (typeof value !== 'object' || value === null) {
      return false;
    }

    const candidate = value as Partial<LedgerBankAccountErrorResponse>;
    return (
  ...
  private mapValidationPayload(
    ...
    const errors: LedgerBankAccountErrorDetail[] = messages.map((message) => ({
  ...
  private inferFieldName(message: string): string {
    const fieldMatch = message.match(/\b(lba[A-Za-z0-9]+)\b/);
    if (fieldMatch) {
      return fieldMatch[1];
    }
