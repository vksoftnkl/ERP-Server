...
@Catch()
export class TenderMasterExceptionFilter implements ExceptionFilter {
    ...
    if (exception instanceof HttpException) {
      ...
      if (this.isTenderMasterErrorResponse(rawResponse)) {
  ...
  private isTenderMasterErrorResponse(value: unknown): value is TenderMasterErrorResponse {
    if (typeof value !== 'object' || value === null) {
      return false;
    }

    const candidate = value as Partial<TenderMasterErrorResponse>;
    return (
  ...
  private mapValidationPayload(payload: ValidationExceptionPayload): TenderMasterErrorResponse {
    ...
    const errors: TenderMasterErrorDetail[] = messages.map((message) => ({
  ...
  private inferFieldName(message: string): string {
    const fieldMatch = message.match(/\b(tnd[A-Za-z0-9]+)\b/);
    if (fieldMatch) {
      return fieldMatch[1];
    }
