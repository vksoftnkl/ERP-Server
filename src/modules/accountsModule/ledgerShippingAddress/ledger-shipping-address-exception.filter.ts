...
@Catch()
export class LedgerShippingAddressExceptionFilter implements ExceptionFilter {
    ...
    if (exception instanceof HttpException) {
      ...
      if (this.isLedgerShippingAddressErrorResponse(rawResponse)) {
  ...
  private isLedgerShippingAddressErrorResponse(
    value: unknown,
  ): value is LedgerShippingAddressErrorResponse {
    if (typeof value !== 'object' || value === null) {
      return false;
    }

    const candidate = value as Partial<LedgerShippingAddressErrorResponse>;
    return (
  ...
  private mapValidationPayload(
    ...
    const errors: LedgerShippingAddressErrorDetail[] = messages.map((message) => ({
  ...
  private inferFieldName(message: string): string {
    const fieldMatch = message.match(/\b(saa[A-Za-z0-9]+)\b/);
    if (fieldMatch) {
      return fieldMatch[1];
    }
