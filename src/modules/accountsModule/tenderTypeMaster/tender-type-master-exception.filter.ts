...
@Catch()
export class TenderTypeMasterExceptionFilter implements ExceptionFilter {
    ...
    if (exception instanceof HttpException) {
      ...
      if (this.isTenderTypeMasterErrorResponse(rawResponse)) {
  ...
  private isTenderTypeMasterErrorResponse(value: unknown): value is TenderTypeMasterErrorResponse {
    if (typeof value !== 'object' || value === null) {
      return false;
    }

    const candidate = value as Partial<TenderTypeMasterErrorResponse>;
    return (
  ...
  private mapValidationPayload(payload: ValidationExceptionPayload): TenderTypeMasterErrorResponse {
    ...
    const errors: TenderTypeMasterErrorDetail[] = messages.map((message) => ({
  ...
  private inferFieldName(message: string): string {
    const fieldMatch = message.match(/\b(ttm[A-Za-z0-9]+)\b/);
    if (fieldMatch) {
      return fieldMatch[1];
    }
