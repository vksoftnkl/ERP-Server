...
@Catch()
export class AccountLedgerMasterExceptionFilter implements ExceptionFilter {
    ...
    if (exception instanceof HttpException) {
      ...
      if (this.isAccountLedgerMasterErrorResponse(rawResponse)) {
  ...
  private isAccountLedgerMasterErrorResponse(
    value: unknown,
  ): value is AccountLedgerMasterErrorResponse {
    if (typeof value !== 'object' || value === null) {
      return false;
    }

    const candidate = value as Partial<AccountLedgerMasterErrorResponse>;
    return (
  ...
  private mapValidationPayload(
    ...
    const errors: AccountLedgerMasterErrorDetail[] = messages.map((message) => ({
  ...
  private inferFieldName(message: string): string {
    const fieldMatch = message.match(/\b(led[A-Za-z0-9]+)\b/);
    if (fieldMatch) {
      return fieldMatch[1];
    }
