...
@Catch()
export class EmployeeDesignationMasterExceptionFilter implements ExceptionFilter {
    ...
    if (exception instanceof HttpException) {
      ...
      if (this.isEmployeeDesignationMasterErrorResponse(rawResponse)) {
  ...
  private isEmployeeDesignationMasterErrorResponse(
    value: unknown,
  ): value is EmployeeDesignationMasterErrorResponse {
    if (typeof value !== 'object' || value === null) {
      return false;
    }

    const candidate = value as Partial<EmployeeDesignationMasterErrorResponse>;
    return (
  ...
  private mapValidationPayload(
    ...
    const errors: EmployeeDesignationMasterErrorDetail[] = messages.map((message) => ({
  ...
  private inferFieldName(message: string): string {
    const fieldMatch = message.match(/\b(ed[A-Za-z0-9]+)\b/);
    if (fieldMatch) {
      return fieldMatch[1];
    }
