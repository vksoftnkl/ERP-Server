...
@Catch()
export class PromotionLoyaltyPointsExceptionFilter implements ExceptionFilter {
  ...
  private isErrorResponse(value: unknown): value is PromotionLoyaltyPointsErrorResponse {
    if (typeof value !== 'object' || value === null) {
      return false;
    }

    const candidate = value as Partial<PromotionLoyaltyPointsErrorResponse>;
    return (
  ...
  private mapValidationPayload(
    ...
    const errors: PromotionLoyaltyPointsErrorDetail[] = messages.map((message) => ({
