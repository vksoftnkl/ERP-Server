...
export function buildListMeta(page: number, limit: number, total: number): PurchaseListMeta {
...
export interface PurchaseListQueryOptions<TRecord, TItem> {
  hasStructuredFilters?: boolean;
  configuredGridFn?: () => Promise<ConfiguredGridListResult<TItem, PurchaseListMeta> | null>;
  countFn: () => Promise<number>;
  findManyFn: () => Promise<TRecord[]>;
  toItemFn: (record: TRecord) => TItem;
  loadStylesFn?: () => Promise<GridColumnItem[] | undefined>;
}
export async function runPurchaseListQuery<TRecord, TItem>(
  pagination: { page: number; limit: number },
  options: PurchaseListQueryOptions<TRecord, TItem>,
): Promise<ConfiguredGridListResult<TItem, PurchaseListMeta>> {
...
): Promise<ConfiguredGridListResult<TItem, PurchaseListMeta> | null> {
