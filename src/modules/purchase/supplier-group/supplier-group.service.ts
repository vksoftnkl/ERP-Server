...
export class SupplierGroupService {
  ...
  private async listFromConfiguredGridSql(
    ...
    const configuredGrid = await this.prisma.gridDetails.findFirst({
      where: {
        ...
        gridSql: {
          not: null,
          contains: SUPPLIER_GROUP_TABLE_NAME,
          mode: 'insensitive',
        },
      },
    ...
    try {
      ...
      const countSql = `SELECT COUNT(*)::bigint AS total FROM (${baseSql}) AS supplier_group_grid`;
      const rowsSql = `SELECT * FROM (${baseSql}) AS supplier_group_grid LIMIT $1 OFFSET $2`;
      const [countResult, rows] = await Promise.all([
        this.prisma.$queryRawUnsafe<Array<{ total: bigint | number | string }>>(countSql),
        this.prisma.$queryRawUnsafe<SupplierGroupListItem[]>(rowsSql, limit, skip),
      ]);
  ...
  private validateConfiguredGridSql(sql: string): string {
    ...
    if (!/\bsupplier_groups\b/i.test(normalized)) {
      this.throwBadRequest('Invalid grid_sql configuration for supplier group list', [
        {
          field: 'grid_sql',
          message: 'Configured query must reference supplier_groups table',
        },
      ]);
    }
