...
export class SuppliersService {
  ...
  private async listFromConfiguredGridSql(
    ...
    try {
      ...
      const countSql = `SELECT COUNT(*)::bigint AS total FROM (${baseSql}) AS supplier_grid`;
      const rowsSql = `SELECT * FROM (${baseSql}) AS supplier_grid LIMIT $1 OFFSET $2`;
      const [countResult, rows] = await Promise.all([
        this.prisma.$queryRawUnsafe<Array<{ total: bigint | number | string }>>(countSql),
        this.prisma.$queryRawUnsafe<SupplierListItem[]>(rowsSql, limit, skip),
      ]);
  ...
  private validateConfiguredGridSql(sql: string): string {
    ...
    if (!/\bsuppliers\b/i.test(normalized)) {
      this.throwBadRequest('Invalid grid_sql configuration for supplier list', [
        {
          field: 'grid_sql',
          message: 'Configured query must reference suppliers table',
        },
      ]);
    }
