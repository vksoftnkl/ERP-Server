...
@Catch()
export class ItemStockBalanceExceptionFilter implements ExceptionFilter {
  ...
  private isItemStockBalanceErrorResponse(value: unknown): value is ItemStockBalanceErrorResponse {
    if (typeof value !== 'object' || value === null) {
      return false;
    }
    const candidate = value as Partial<ItemStockBalanceErrorResponse>;
    return (
  ...
  private mapValidationPayload(payload: ValidationExceptionPayload): ItemStockBalanceErrorResponse {
    const messages = Array.isArray(payload.message)
      ? payload.message
      : payload.message
        ? [payload.message]
        : ['Validation failed'];
    const errors: ItemStockBalanceErrorDetail[] = messages.map((message) => ({
  ...
  private inferFieldName(message: string): string {
    const fieldMatch = message.match(
      /\b(isb_acc_year|isb_company_id|isb_branch_id|isb_godown_id|isb_item_id|isb_unit_id|isb_stock_bucket)\b/i,
    );
    if (fieldMatch) {
      return fieldMatch[1];
    }
