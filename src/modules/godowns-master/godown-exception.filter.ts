...
@Catch()
export class GodownExceptionFilter implements ExceptionFilter {
    ...
    if (exception instanceof HttpException) {
      ...
      if (this.isGodownErrorResponse(rawResponse)) {
  ...
  private isGodownErrorResponse(value: unknown): value is GodownErrorResponse {
    if (typeof value !== 'object' || value === null) {
      return false;
    }

    const candidate = value as Partial<GodownErrorResponse>;
    return (
  ...
  private mapValidationPayload(payload: ValidationExceptionPayload): GodownErrorResponse {
    ...
    const errors: GodownErrorDetail[] = messages.map((message) => ({
  ...
  private inferFieldName(message: string): string {
    const fieldMatch = message.match(/\b((?:gdl|godown)_[a-z0-9_]+)\b/i);
    if (fieldMatch) {
      return fieldMatch[1];
    }
