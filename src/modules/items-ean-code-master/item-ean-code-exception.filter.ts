...
@Catch()
export class ItemEanCodeExceptionFilter implements ExceptionFilter {
    ...
    if (exception instanceof HttpException) {
      ...
      if (this.isItemEanCodeErrorResponse(rawResponse)) {
  ...
  private isItemEanCodeErrorResponse(value: unknown): value is ItemEanCodeErrorResponse {
    if (typeof value !== 'object' || value === null) {
      return false;
    }

    const candidate = value as Partial<ItemEanCodeErrorResponse>;
    return (
  ...
  private mapValidationPayload(payload: ValidationExceptionPayload): ItemEanCodeErrorResponse {
    ...
    const errors: ItemEanCodeErrorDetail[] = messages.map((message) => ({
  ...
  private inferFieldName(message: string): string {
    const fieldMatch = message.match(/\b(ean_[a-z0-9_]+)\b/i);
    if (fieldMatch) {
      return fieldMatch[1];
    }
