...
describe('ItemTaxExceptionFilter', () => {
  let filter: ItemTaxExceptionFilter;
  let status: jest.Mock;
  let json: jest.Mock;
  let response: Response;

  beforeEach(() => {
    filter = new ItemTaxExceptionFilter();
    json = jest.fn();
  ...
  it('passes through standardized error payloads', () => {
    const exception = new NotFoundException({
      success: false,
      message: 'Item tax not found',
      errors: [{ field: 'tax_id', message: 'Not found' }],
    });
  ...
  it('maps validation payload to standard error format', () => {
    const exception = new BadRequestException({
      message: ['tax_name should not be empty'],
    });
