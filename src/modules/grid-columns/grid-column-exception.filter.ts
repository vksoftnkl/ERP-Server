...
import { GridColumnErrorDetail, GridColumnErrorResponse } from './types/grid-column-api.types';
...
@Catch()
export class GridColumnExceptionFilter implements ExceptionFilter {
    ...
    if (exception instanceof HttpException) {
      ...
      if (this.isGridColumnErrorResponse(rawResponse)) {
  ...
  private isGridColumnErrorResponse(value: unknown): value is GridColumnErrorResponse {
    if (typeof value !== 'object' || value === null) {
      return false;
    }

    const candidate = value as Partial<GridColumnErrorResponse>;
    return (
  ...
  private mapValidationPayload(payload: ValidationExceptionPayload): GridColumnErrorResponse {
    ...
    const errors: GridColumnErrorDetail[] = messages.map((message) => ({
  ...
  private inferFieldName(message: string): string {
    const fieldMatch = message.match(/\b(grid_column_[a-z0-9_]+|grid_[a-z0-9_]+)\b/i);
    if (fieldMatch) {
      return fieldMatch[1];
    }
