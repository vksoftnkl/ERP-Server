...
@Catch()
export class ItemPriceExceptionFilter implements ExceptionFilter {
    ...
    if (exception instanceof HttpException) {
      ...
      if (this.isItemPriceErrorResponse(rawResponse)) {
  ...
  private isItemPriceErrorResponse(value: unknown): value is ItemPriceErrorResponse {
    if (typeof value !== 'object' || value === null) {
      return false;
    }

    const candidate = value as Partial<ItemPriceErrorResponse>;
    return (
  ...
  private mapValidationPayload(payload: ValidationExceptionPayload): ItemPriceErrorResponse {
    ...
    const errors: ItemPriceErrorDetail[] = messages.map((message) => ({
  ...
  private inferFieldName(message: string): string {
    const fieldMatch = message.match(/\b(ipm_[a-z0-9_]+)\b/i);
    if (fieldMatch) {
      return fieldMatch[1];
    }
