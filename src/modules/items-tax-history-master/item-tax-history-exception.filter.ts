...
@Catch()
export class ItemTaxHistoryExceptionFilter implements ExceptionFilter {
    ...
    if (exception instanceof HttpException) {
      ...
      if (this.isItemTaxHistoryErrorResponse(rawResponse)) {
  ...
  private isItemTaxHistoryErrorResponse(value: unknown): value is ItemTaxHistoryErrorResponse {
    if (typeof value !== 'object' || value === null) {
      return false;
    }

    const candidate = value as Partial<ItemTaxHistoryErrorResponse>;
    return (
  ...
  private mapValidationPayload(payload: ValidationExceptionPayload): ItemTaxHistoryErrorResponse {
    ...
    const errors: ItemTaxHistoryErrorDetail[] = messages.map((message) => ({
  ...
  private inferFieldName(message: string): string {
    const fieldMatch = message.match(/\b(ith_[a-z0-9_]+)\b/i);
    if (fieldMatch) {
      return fieldMatch[1];
    }
