...
@Catch()
export class ItemQtywiseRateExceptionFilter implements ExceptionFilter {
    ...
    if (exception instanceof HttpException) {
      ...
      if (this.isItemQtywiseRateErrorResponse(rawResponse)) {
  ...
  private isItemQtywiseRateErrorResponse(value: unknown): value is ItemQtywiseRateErrorResponse {
    if (typeof value !== 'object' || value === null) {
      return false;
    }

    const candidate = value as Partial<ItemQtywiseRateErrorResponse>;
    return (
  ...
  private mapValidationPayload(payload: ValidationExceptionPayload): ItemQtywiseRateErrorResponse {
    ...
    const errors: ItemQtywiseRateErrorDetail[] = messages.map((message) => ({
  ...
  private inferFieldName(message: string): string {
    const fieldMatch = message.match(/\b(iqr_[a-z0-9_]+)\b/i);
    if (fieldMatch) {
      return fieldMatch[1];
    }
