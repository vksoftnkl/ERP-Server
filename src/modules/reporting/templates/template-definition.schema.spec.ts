import { templateDefinitionSchema } from './dto/template-definition.schema';
...
describe('templateDefinitionSchema invariants', () => {
  it('accepts every shipped gallery template', () => {
    // The gallery is the standing proof that the schema and the engine agree.
    for (const entry of GALLERY_TEMPLATES) {
      const result = parse(entry.build());
      if (!result.success) {
        throw new Error(
          `${entry.key} failed: ${result.error.issues.map((issue) => `${issue.path.join('.')}: ${issue.message}`).join('; ')}`,
        );
      }
