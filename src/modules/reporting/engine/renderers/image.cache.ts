 ...
 *     operator makes deliberately via REPORT_ALLOW_REMOTE_IMAGES, not a
...
export class ImageCache {
  ...
  constructor() {
    ...
    this.allowRemote = ['1', 'true', 'yes', 'on'].includes(
      (process.env.REPORT_ALLOW_REMOTE_IMAGES ?? '').toLowerCase(),
    );
  ...
  private async load(source: string): Promise<Buffer | null> {
    ...
    if (/^https?:\/\//i.test(source)) {
      if (!this.allowRemote) {
        this.warn(
          `Remote image '${truncateForLog(source)}' refused. Set REPORT_ALLOW_REMOTE_IMAGES=true ` +
 ...
 *
 * REPORT_IMAGE_ROOTS is a path-separator-joined list. The default is the
