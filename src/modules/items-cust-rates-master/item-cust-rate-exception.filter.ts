...
@Catch()
export class ItemCustRateExceptionFilter implements ExceptionFilter {
  ...
  private isItemCustRateErrorResponse(value: unknown): value is ItemCustRateErrorResponse {
    if (typeof value !== 'object' || value === null) {
      return false;
    }

    const candidate = value as Partial<ItemCustRateErrorResponse>;
    return (
  ...
  private mapValidationPayload(payload: ValidationExceptionPayload): ItemCustRateErrorResponse {
    ...
    const errors: ItemCustRateErrorDetail[] = messages.map((message) => ({
