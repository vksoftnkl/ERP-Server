...
export class UnitExceptionFilter implements ExceptionFilter {
  ...
  private isUnitErrorResponse(value: unknown): value is UnitErrorResponse {
    if (typeof value !== 'object' || value === null) {
      return false;
    }

    const candidate = value as Partial<UnitErrorResponse>;
    return (
  ...
  private mapValidationPayload(payload: ValidationExceptionPayload): UnitErrorResponse {
    ...
    const errors: UnitErrorDetail[] = messages.map((message) => ({
