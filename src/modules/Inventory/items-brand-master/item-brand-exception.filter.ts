...
@Catch()
export class ItemBrandExceptionFilter implements ExceptionFilter {
    ...
    if (exception instanceof HttpException) {
      ...
      if (this.isItemBrandErrorResponse(rawResponse)) {
  ...
  private isItemBrandErrorResponse(value: unknown): value is ItemBrandErrorResponse {
    if (typeof value !== 'object' || value === null) {
      return false;
    }

    const candidate = value as Partial<ItemBrandErrorResponse>;
    return (
  ...
  private mapValidationPayload(payload: ValidationExceptionPayload): ItemBrandErrorResponse {
    ...
    const errors: ItemBrandErrorDetail[] = messages.map((message) => ({
  ...
  private inferFieldName(message: string): string {
    const fieldMatch = message.match(/\b(brand_[a-z0-9_]+)\b/i);
    if (fieldMatch) {
      return fieldMatch[1];
    }
