...
@Catch()
export class ItemGroupExceptionFilter implements ExceptionFilter {
    ...
    if (exception instanceof HttpException) {
      ...
      if (this.isItemGroupErrorResponse(rawResponse)) {
  ...
  private isItemGroupErrorResponse(value: unknown): value is ItemGroupErrorResponse {
    if (typeof value !== 'object' || value === null) {
      return false;
    }

    const candidate = value as Partial<ItemGroupErrorResponse>;
    return (
  ...
  private mapValidationPayload(payload: ValidationExceptionPayload): ItemGroupErrorResponse {
    ...
    const errors: ItemGroupErrorDetail[] = messages.map((message) => ({
  ...
  private inferFieldName(message: string): string {
    const fieldMatch = message.match(/\b(itg_[a-z0-9_]+)\b/i);
    if (fieldMatch) {
      return fieldMatch[1];
    }
