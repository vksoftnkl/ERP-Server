...
describe('ItemEanCodeExceptionFilter', () => {
  let filter: ItemEanCodeExceptionFilter;
  let status: jest.Mock;
  let json: jest.Mock;
  let response: Response;

  beforeEach(() => {
    filter = new ItemEanCodeExceptionFilter();
    json = jest.fn();
  ...
  it('passes through standardized error payloads', () => {
    const exception = new NotFoundException({
      success: false,
      message: 'Item EAN code not found',
      errors: [{ field: 'ean_id', message: 'Not found' }],
    });
  ...
  it('maps validation payload to standard error format', () => {
    const exception = new BadRequestException({
      message: ['ean_code should not be empty'],
    });
