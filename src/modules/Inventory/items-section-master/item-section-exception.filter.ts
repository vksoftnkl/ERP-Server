...
@Catch()
export class ItemSectionExceptionFilter implements ExceptionFilter {
    ...
    if (exception instanceof HttpException) {
      ...
      if (this.isItemSectionErrorResponse(rawResponse)) {
  ...
  private isItemSectionErrorResponse(value: unknown): value is ItemSectionErrorResponse {
    if (typeof value !== 'object' || value === null) {
      return false;
    }

    const candidate = value as Partial<ItemSectionErrorResponse>;
    return (
  ...
  private mapValidationPayload(payload: ValidationExceptionPayload): ItemSectionErrorResponse {
    ...
    const errors: ItemSectionErrorDetail[] = messages.map((message) => ({
  ...
  private inferFieldName(message: string): string {
    const fieldMatch = message.match(/\b(sec_[a-z0-9_]+)\b/i);
    if (fieldMatch) {
      return fieldMatch[1];
    }
