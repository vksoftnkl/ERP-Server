...
describe('GodownExceptionFilter', () => {
  let filter: GodownExceptionFilter;
  let status: jest.Mock;
  let json: jest.Mock;
  let response: Response;

  beforeEach(() => {
    filter = new GodownExceptionFilter();
    json = jest.fn();
  ...
  it('passes through standardized error payloads', () => {
    const exception = new NotFoundException({
      success: false,
      message: 'Godown location not found',
      errors: [{ field: 'gdl_id', message: 'Not found' }],
    });
  ...
  it('maps validation payload to standard error format', () => {
    const exception = new BadRequestException({
      message: ['gdl_name should not be empty'],
    });
