...
export class SaveTxnHoldDto {
  ...
  // normalises rather than letting the constraint reject the row — and nullable
  // rather than optional, so a blank string lands as NULL instead of failing
  // the constraint's length check.
  @ApiPropertyOptional({
    maxLength: 30,
    nullable: true,
    description:
      'Printed token number, generated by the till. Optional — only a HOLD prints a slip, so ' +
