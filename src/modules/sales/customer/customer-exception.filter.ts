...
export class CustomerExceptionFilter implements ExceptionFilter {
    ...
    if (exception instanceof HttpException) {
      ...
      if (this.isCustomerErrorResponse(rawResponse)) {
  ...
  private isCustomerErrorResponse(value: unknown): value is CustomerErrorResponse {
    if (typeof value !== 'object' || value === null) {
      return false;
    }

    const candidate = value as Partial<CustomerErrorResponse>;
    return (
  ...
  private mapValidationPayload(payload: ValidationExceptionPayload): CustomerErrorResponse {
    ...
    const errors: CustomerErrorDetail[] = messages.map((message) => ({
  ...
  private inferFieldName(message: string): string {
    const fieldMatch = message.match(/\b(cus[A-Za-z0-9]+)\b/);
    if (fieldMatch) {
      return fieldMatch[1];
    }
