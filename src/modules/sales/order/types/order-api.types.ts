...
// sale_charge_detail is polymorphic — an order's applied charges are the rows
// carrying this discriminator plus cdDocId = soId. ORDER was added to
// ChargeDocType alongside this module; sale_charge_detail carries no
// ck_cd_doc_type CHECK any more, so the enum alone defines the allowed set.
export const ORDER_CHARGE_DOC_TYPE = ChargeDocType.ORDER;
...
export const ORDER_CHARGE_AUDIT: ChargeDocumentAudit = {
...
export const ORDER_TENDER_SRC_MODULE = TenderSrcModule.SALES;
export const ORDER_TENDER_SRC_DOC_TYPE = TenderSrcDocType.SALES_ORDER;
...
// business receives.
export const ORDER_TENDER_DR_CR = TenderDrCr.DR;
...
export const ORDER_TENDER_AUDIT: TenderDocumentAudit = {
...
> & {
  ...
  charges?: OrderChargePayload[];
  tenders?: OrderTenderPayload[];
  advances?: OrderAdvancePayload[];
};
...
// Prisma returns it.
export type OrderAdvancePayload = Omit<
...
// ledger's name.
export type OrderChargePayload = ChargeDetailPayload;
// Likewise for a tendered amount: the tender-detail module's payload verbatim.
export type OrderTenderPayload = TenderDetailPayload;
export type OrderErrorDetail = {
  field: string;
  message: string;
};
export type OrderErrorResponse = {
  ...
  success: false;
  message: string;
  errors: OrderErrorDetail[];
};
export type OrderSuccessResponse<T> = {
  success: true;
  message: string;
  data: T;
};
