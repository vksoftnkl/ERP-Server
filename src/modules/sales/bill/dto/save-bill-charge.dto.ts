...
// One applied charge line on a bill (sale_charge_detail row with
// cdDocType = 'INVOICE' — a bill IS the tax invoice). Every cd* value except
// the amounts is a SNAPSHOT of the charge master taken at save time, so the
// client sends them explicitly rather than the server re-reading the master on
// each save.
export class SaveBillChargeDto {
  @ApiPropertyOptional({
    format: 'uuid',
  ...
  @ApiPropertyOptional({
    nullable: true,
    description: "Defaults to the bill's sbBillSlno",
  })
  @NullableNumber()
  ...
  @OptionalBoolean()
  cdIsActive?: boolean;
  // cd_created_by / cd_modified_by are text columns — the actor is whatever
  // resolveActor settles on — a user id, but equally a name or login the caller
  // passed — so the uuid pattern does not fit. Free text, no length cap,
  // matching the column and the header's sbCreatedBy / sbModifiedBy.
