 ...
 * is_deleted = false AND is_active = true, on the header and on all four
 * grids.
 *
 * `company` and `branch` are accepted as aliases for prm_comp_id and
 * prm_branch_id — the query string may use either spelling, and both are
 * whitelisted so forbidNonWhitelisted does not reject the short form.
 *
 * The `@Expose()` on the two canonical fields is load-bearing, not decoration:
 ...
 */
export class ListPromotionSchemeQueryDto {
  @ApiPropertyOptional({
  ...
  @Matches(UUID_PATTERN, { message: 'prm_comp_id must be a valid UUID' })
  prm_comp_id?: string;

  @ApiPropertyOptional({
    example: '01963d86-caf0-7b26-89f0-58ac380a2d5e',
    description:
      'branch_master.br_id — optional. Matches the prm_branch_id column literally: omit it to ' +
  ...
  @Matches(UUID_PATTERN, { message: 'prm_branch_id must be a valid UUID' })
  prm_branch_id?: string;
