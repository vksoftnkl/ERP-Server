...
@Catch()
export class CityExceptionFilter implements ExceptionFilter {
    ...
    if (exception instanceof HttpException) {
      ...
      if (this.isCityErrorResponse(rawResponse)) {
  ...
  private isCityErrorResponse(value: unknown): value is CityErrorResponse {
    if (typeof value !== 'object' || value === null) {
      return false;
    }

    const candidate = value as Partial<CityErrorResponse>;
    return (
  ...
  private mapValidationPayload(payload: ValidationExceptionPayload): CityErrorResponse {
    ...
    const errors: CityErrorDetail[] = messages.map((message) => ({
  ...
  private inferFieldName(message: string): string {
    const fieldMatch = message.match(/\b(ctm[A-Za-z0-9]+)\b/);
    if (fieldMatch) {
      return fieldMatch[1];
    }
