...
@Catch()
export class CustomerGroupExceptionFilter implements ExceptionFilter {
    ...
    if (exception instanceof HttpException) {
      ...
      if (this.isCustomerGroupErrorResponse(rawResponse)) {
  ...
  private isCustomerGroupErrorResponse(value: unknown): value is CustomerGroupErrorResponse {
    if (typeof value !== 'object' || value === null) {
      return false;
    }

    const candidate = value as Partial<CustomerGroupErrorResponse>;
    return (
  ...
  private mapValidationPayload(payload: ValidationExceptionPayload): CustomerGroupErrorResponse {
    ...
    const errors: CustomerGroupErrorDetail[] = messages.map((message) => ({
  ...
  private inferFieldName(message: string): string {
    const fieldMatch = message.match(/\b(cgr[A-Za-z0-9]+)\b/);
    if (fieldMatch) {
      return fieldMatch[1];
    }
