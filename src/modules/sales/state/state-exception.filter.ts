...
@Catch()
export class StateExceptionFilter implements ExceptionFilter {
    ...
    if (exception instanceof HttpException) {
      ...
      if (this.isStateErrorResponse(rawResponse)) {
  ...
  private isStateErrorResponse(value: unknown): value is StateErrorResponse {
    if (typeof value !== 'object' || value === null) {
      return false;
    }

    const candidate = value as Partial<StateErrorResponse>;
    return (
  ...
  private mapValidationPayload(payload: ValidationExceptionPayload): StateErrorResponse {
    ...
    const errors: StateErrorDetail[] = messages.map((message) => ({
  ...
  private inferFieldName(message: string): string {
    const fieldMatch = message.match(/\b(stm[A-Za-z0-9]+)\b/);
    if (fieldMatch) {
      return fieldMatch[1];
    }
